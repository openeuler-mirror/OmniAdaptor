...
public class ClosureCleaner {
    ...
    private static volatile Pattern SKIP_CLASS_PATTERN;

    public static void setSkipClassRegex(String regex) {
        if (regex == null || regex.isEmpty()) {
            SKIP_CLASS_PATTERN = null;
            LOG.info("SKIP_CLASS_PATTERN is null.");
        } else {
            SKIP_CLASS_PATTERN = Pattern.compile(regex);
            LOG.info("SKIP_CLASS_PATTERN is {}.", SKIP_CLASS_PATTERN.toString());
        }
    }

    private static boolean shouldSkipClass(String className) {
        Pattern p = SKIP_CLASS_PATTERN;
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "shouldSkipClass: the className is {}, the pattern is {}, the res is {}",
                    className,
                    p.toString(),
                    p.matcher(className).matches());
        }
        return p != null && p.matcher(className).matches();
    }
    ...
    private static void clean(
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Start to clean the class: {}", className);
        }
        for (Field f : cls.getDeclaredFields()) {
            if (f.getName().startsWith("this$")) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Start to execute cleanThis0 for {}.", className);
                }
                if (shouldSkipClass(className)) {
                    LOG.info("Skip closure cleaning by regex for class {}", className);
                    return;
                }
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Clean not starts with this$ {}", className);
                }
                Object fieldObject;
                try {
    ...
    private static ClassReader getClassReader(Class<?> cls) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Could not create ClassReader for " + cls.getName() + " :" + e.getMessage(), e);
        }
    }
