...
class PartitionRequestServerHandler extends SimpleChannelInboundHandler<NettyMessage> {
        ...
        try {
            ...
            if (msgClazz == PartitionRequest.class) {
                ...
                try {
                    ...
                    if (isNative) {
                        reader =
                                new OmniCreditBasedSequenceNumberingViewReader(
    ...
    /**
     * checkIfCreditBasedSequenceNumberingViewReaderIsNative
     *
     * @param partitionId partitionId
     * @return boolean
     */
    public boolean checkIfCreditBasedSequenceNumberingViewReaderIsNative(ResultPartitionID partitionId) throws PartitionNotFoundException {
        boolean flag = false;

        if (this.partitionProvider instanceof ResultPartitionManager) {
            ResultPartitionManager resultPartitionManager = (ResultPartitionManager) this.partitionProvider;
            Class clazz = ResultPartitionManager.class;
            try {
                Field field = clazz.getDeclaredField("registeredPartitions");
                ...
                if (resultPartition instanceof BufferWritingResultPartition) {
                    ...
                    flag = bufferWritingResultPartition.getNativeTaskRef() != -1;
                }
