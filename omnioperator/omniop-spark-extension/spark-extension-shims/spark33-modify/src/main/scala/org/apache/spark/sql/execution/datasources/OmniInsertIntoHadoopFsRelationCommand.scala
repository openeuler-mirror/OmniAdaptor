 ...
 */
case class OmniInsertIntoHadoopFsRelationCommand(
                                              outputPath: Path,
  ...
  // Return Seq[Row] but Seq[ColumBatch] since
  // 1. reuse the origin interface of spark to avoid add duplicate code
  // 2. this func return a Seq.empty[Row] and this data doesn't do anything else
    ...
    if (doInsertion) {
      ...
      val updatedPartitionPaths =
        OmniFileFormatWriter.write(
          ...
          fileFormat = fileFormat,
          committer = committer,
          outputSpec = OmniFileFormatWriter.OutputSpec(
  ...
  OmniInsertIntoHadoopFsRelationCommand = copy(query = newChild)
}
