...
case class ColumnarBroadcastHashJoinExec(
  ...
  def buildCheck(): Unit = {
    if (isNullAwareAntiJoin) {
      throw new UnsupportedOperationException(s"isNullAwareAntiJoin is not supported " +
        s"in ${this.nodeName}")
    }
    joinType match {
      case LeftOuter | Inner | LeftSemi | LeftAnti | RightOuter =>
      case _ =>
  ...
  override def doExecuteColumnar(): RDD[ColumnarBatch] = {
    ...
    streamedPlan.executeColumnar().mapPartitionsWithIndexInternal { (index, iter) =>
      ...
      def createBuildOpFactoryAndOp(isShared: Boolean): (OmniHashBuilderWithExprOperatorFactory, OmniOperator) = {
        ...
        if (isShared) {
          OmniHashBuilderWithExprOperatorFactory.saveHashBuilderOperatorAndFactory(buildPlanId,
            opFactory, op)
        }
        ...
        try {
          ...
          case e: Exception => {
            if (isShared) {
              OmniHashBuilderWithExprOperatorFactory.dereferenceHashBuilderOperatorAndFactory(buildPlanId)
            } else {
              op.close()
              opFactory.close()
            }
      ...
      if (enableShareBuildOp && canShareBuildOp) {
        ...
        try {
          buildOpFactory = OmniHashBuilderWithExprOperatorFactory.getHashBuilderOperatorFactory(buildPlanId)
      ...
      val lookupOpFactory = new OmniLookupJoinWithExprOperatorFactory(probeTypes, probeOutputCols,
        probeHashColsExp, buildOutputCols, buildOutputTypes, buildOpFactory, filter,
        new OperatorConfig(SpillConfig.NONE,
          new OverflowConfig(OmniAdaptorUtil.overflowConf()), IS_SKIP_VERIFY_EXP))
