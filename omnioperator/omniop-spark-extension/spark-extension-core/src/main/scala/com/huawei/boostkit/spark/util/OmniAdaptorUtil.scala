...
object OmniAdaptorUtil {
  def copyVecBatch(vb: VecBatch): VecBatch = {
    val vecCount = vb.getVectorCount
    val rowCount = vb.getRowCount
    val vecs = new Array[Vec](vecCount)
    for (index <- 0 until vecCount) {
      vecs(index) = vb.getVector(index).slice(0, rowCount)
    }
    new VecBatch(vecs, rowCount)
  }
  ...
  def genSortParam(output: Seq[Attribute], sortOrder: Seq[SortOrder]):
    ...
    if (inputColSize == 0) {
      throw new UnsupportedOperationException("input column size should not be 0!")
    }
    val sourceTypes = new Array[nova.hetu.omniruntime.`type`.DataType](inputColSize)
    val ascending = new Array[Int](sortOrder.size)
    ...
    sortOrder.zipWithIndex.foreach { case (sortAttr, i) =>
      ...
      ascending(i) = if (sortAttr.isAscending) {
        1
      } else {
        0
      }
    ...
    (sourceTypes, ascending, nullFirsts, sortColsExp)
  }

  def addAllAndGetIterator(operator: OmniOperator,
                           inputIter: Iterator[ColumnarBatch], schema: StructType,
                           addInputTime: SQLMetric, numInputVecBatches: SQLMetric,
                           numInputRows: SQLMetric, getOutputTime: SQLMetric,
                           numOutputVecBatches: SQLMetric, numOutputRows: SQLMetric,
  ...
  private def GetStatisticalAggregate(): Boolean = {
  ...
  def getAggOperator(groupingExpressions: Seq[NamedExpression],
                     ...
                     sparkSpillConf: SpillConfig = SpillConfig.NONE):
  (OmniOperator, OmniHashAggregationWithExprOperatorFactory, OmniAggregationWithExprOperatorFactory) = {
    var hashAggregationWithExprOperatorFactory: OmniHashAggregationWithExprOperatorFactory = null
    var aggregationWithExprOperatorFactory : OmniAggregationWithExprOperatorFactory = null
    var operator: OmniOperator = null
    if (groupingExpressions.nonEmpty) {
      hashAggregationWithExprOperatorFactory = new OmniHashAggregationWithExprOperatorFactory(
        omniGroupByChanel,
        omniAggChannels,
        ...
        omniInputRaws,
        omniOutputPartials,
        new OperatorConfig(sparkSpillConf, GetStatisticalAggregate(),
      ...
      aggregationWithExprOperatorFactory = new OmniAggregationWithExprOperatorFactory(
        ...
        omniInputRaws,
        omniOutputPartials,
        new OperatorConfig(SpillConfig.NONE, GetStatisticalAggregate(),
          new OverflowConfig(OmniAdaptorUtil.overflowConf()), IS_SKIP_VERIFY_EXP))
      operator = aggregationWithExprOperatorFactory.createOperator
    }
    (operator, hashAggregationWithExprOperatorFactory, aggregationWithExprOperatorFactory)
  }
