...
object PhysicalPlanSelector extends QueryPlanSelector[SparkPlan] {
  override protected def validate(plan: SparkPlan): Boolean = true
}
...
abstract class QueryPlanSelector[T <: QueryPlan[_]] extends Logging {
  ...
  private[this] def shouldUseOmni(session: SparkSession, plan: T): Boolean = {
    ...
    logInfo(s"shouldUseOmni: $omniEnabled")
    omniEnabled & validate(plan)
  }

  def maybe(session: SparkSession, plan: T)(func: => T): T = {
    if (shouldUseOmni(session, plan)) func else plan
  }

  def maybeNil(session: SparkSession, plan: T)(func: => Seq[SparkPlan]): Seq[SparkPlan] = {
    if (shouldUseOmni(session, plan)) func else Nil
  }
}