...
import org.apache.spark.{SPARK_VERSION_SHORT, SparkEnv, TaskContext, TaskContextImpl}
...
object ShimUtil {
  ...
  private val LOW_SPARK_VERSION_SET: Set[String] = Set("3.4.0", "3.4.1")

  private val cteRelationDefConstructor: Constructor[CTERelationRef] = {
    val clazz = classOf[CTERelationRef]
    if (LOW_SPARK_VERSION_SET.contains(SPARK_VERSION_SHORT)) {
      clazz.getDeclaredConstructor(classOf[Long], classOf[Boolean], classOf[Seq[Attribute]], classOf[Option[Statistics]])
    } else {
      clazz.getDeclaredConstructor(classOf[Long], classOf[Boolean], classOf[Seq[Attribute]], classOf[Boolean], classOf[Option[Statistics]])
    }
  }
  ...
  def createCTERelationRef(cteId: Long, resolved: Boolean, output: Seq[Attribute], isStreaming: Boolean,
    ...
    if (LOW_SPARK_VERSION_SET.contains(SPARK_VERSION_SHORT)) {
      cteRelationDefConstructor.newInstance(cteId.asInstanceOf[AnyRef], resolved.asInstanceOf[AnyRef], output.asInstanceOf[AnyRef], tatsOpt.asInstanceOf[AnyRef])
    } else {
      cteRelationDefConstructor.newInstance(cteId.asInstanceOf[AnyRef], resolved.asInstanceOf[AnyRef], output.asInstanceOf[AnyRef], isStreaming.asInstanceOf[AnyRef], tatsOpt.asInstanceOf[AnyRef])
    }
  }
  ...
  def binaryOperatorAdjust(expr: BinaryOperator, returnDataType: DataType): (Expression, Expression) = {
    ...
    if (DecimalType.unapply(expr.left) && DecimalType.unapply(expr.right)) {
