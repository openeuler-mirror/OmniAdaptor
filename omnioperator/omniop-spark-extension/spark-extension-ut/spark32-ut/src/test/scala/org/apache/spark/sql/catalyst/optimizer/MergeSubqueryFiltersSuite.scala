...
class MergeSubqueryFiltersSuite extends PlanTest {
  ...
  override def beforeEach(): Unit = {
    val curIdFiled = CTERelationDef.getClass.getDeclaredField("curId")
    val moduleFiled = CTERelationDef.getClass.getDeclaredField("MODULE$")
    moduleFiled.setAccessible(true)
    curIdFiled.setAccessible(true)
    val module = moduleFiled.get(null)
    curIdFiled.get(module).asInstanceOf[java.util.concurrent.atomic.AtomicLong].set(0)
  }
