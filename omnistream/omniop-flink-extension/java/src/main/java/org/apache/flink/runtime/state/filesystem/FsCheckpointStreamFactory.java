 ...
 *
 * <p>This copy of the Flink class adds {@link ByteBufferWritable} support to
 * {@link FsCheckpointStateOutputStream}, enabling zero-copy writes from OmniStream
 * native DirectByteBuffers during savepoint output. Other logic is unchanged from
 * the upstream Flink implementation.
 */
public class FsCheckpointStreamFactory implements CheckpointStreamFactory {
     ...
     *
     * <p>Extends the standard Flink implementation with {@link ByteBufferWritable} support
     * to enable zero-copy writes from native DirectByteBuffers during OmniStream savepoints.
     * When the underlying FSDataOutputStream supports ByteBufferWritable, the write path
     * passes the buffer directly; otherwise it falls back to heap-array copying.
     */
    public static class FsCheckpointStateOutputStream extends CheckpointStateOutputStream
        ...
        public boolean write(ByteBuffer buffer) throws IOException {
            ...
            if (buffer.hasArray()) {
                write(buffer.array(), buffer.arrayOffset() + buffer.position(), len);
                buffer.position(buffer.position() + len);
                return false;
            }
            if (len < writeBuffer.length) {
                copyByteBufferToInternalBuffer(buffer, len);
                return false;
            }
            flushToFile();
            if (outStream instanceof ByteBufferWritable) {
                return ((ByteBufferWritable) outStream).write(buffer);
            }
            writeByteBufferWithFallbackCopy(buffer);
            return false;
        }
        ...
        private void copyByteBufferToInternalBuffer(ByteBuffer buffer, int len) throws IOException {
            final int remaining = writeBuffer.length - pos;
            if (len > remaining) {
                buffer.get(writeBuffer, pos, remaining);
                pos += remaining;
                len -= remaining;
                flushToFile();
            }
            buffer.get(writeBuffer, pos, len);
            pos += len;
        }

        private void writeByteBufferWithFallbackCopy(ByteBuffer buffer) throws IOException {
            byte[] copyBuffer = getByteBufferFallbackBuffer();
            while (buffer.hasRemaining()) {
                int len = Math.min(buffer.remaining(), copyBuffer.length);
                buffer.get(copyBuffer, 0, len);
                outStream.write(copyBuffer, 0, len);
            }
        }

        private byte[] getByteBufferFallbackBuffer() {
            if (byteBufferFallbackBuffer == null) {
                byteBufferFallbackBuffer = new byte[writeBuffer.length];
            }
