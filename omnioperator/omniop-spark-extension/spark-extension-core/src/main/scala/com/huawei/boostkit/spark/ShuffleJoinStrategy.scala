...
import org.apache.spark.sql.execution.{ExtractEquiJoinKeysShim, SparkPlan, joins}
...
object ShuffleJoinStrategy extends Strategy
  ...
  private val columnarForceShuffledHashJoin =
    ColumnarPluginConfig.getConf.forceShuffledHashJoin
      ...
      case ExtractEquiJoinKeysShim(joinType, leftKeys, rightKeys, nonEquiCond, left, right, hint)
        ...
        if (enable) {
          ...
          if (conf.cboEnabled) {
            ...
            if (columnarForceShuffledHashJoin) {
              leftBuildable = canBuildShuffledHashJoinLeft(joinType)
