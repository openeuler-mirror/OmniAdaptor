...
import org.apache.spark.sql.catalyst.expressions.{ListQuery, SubqueryExpression, RuntimeFilterSubquery, DynamicPruningSubquery}
...
case class InsertAdaptiveSparkPlan(
  ...
  private def buildSubqueryMap(plan: SparkPlan): Map[Long, SparkPlan] = {
    ...
    if (!plan.containsAnyPattern(SCALAR_SUBQUERY, IN_SUBQUERY, DYNAMIC_PRUNING_SUBQUERY, RUNTIME_FILTER_SUBQUERY)) {
