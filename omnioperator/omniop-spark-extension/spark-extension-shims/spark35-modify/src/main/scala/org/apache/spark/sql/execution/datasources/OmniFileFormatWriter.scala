...
import org.apache.spark.sql.execution.{ColumnarProjectExec, ColumnarSortExec, OmniColumnarToRowExec, ProjectExec, SQLExecution, SortExec, SparkPlan, UnsafeExternalRowSorter}
...
object OmniFileFormatWriter extends Logging {
  ...
  case class Empty2Null(child: Expression) extends UnaryExpression with String2StringExpression {
    ...
    override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
      throw new UnsupportedOperationException("This operator doesn't support doGenCode")
    }
  ...
  : Set[String] = {
    ...
    val empty2NullPlan = if (needConvert) ColumnarProjectExec(projectList, plan) else plan
    ...
    try {
      val (rdd, concurrentOutputWriterSpec) = if (orderingMatched) {
        ...
        //        val orderingExpr = requiredOrdering.map(SortOrder(_, Ascending))
        val sortPlan = ColumnarSortExec(
          orderingExpr,
          global = false,
        ...
        if (concurrentWritersEnabled) {
          // TODO  Concurrent output write
          logInfo("Columnar concurrent write is not support now, use un concurrent write")
          (sortPlan.executeColumnar(), None)
        } else {
          (sortPlan.executeColumnar(), None)
        }
      }
      ...
      val rddWithNonEmptyPartitions = if (rdd.partitions.length == 0) {
        sparkSession.sparkContext.parallelize(Array.empty[OmniInternalRow], 1)
      } else {
        rdd.map(cb => new OmniInternalRow(cb))
      }
  ...
  private def executeTask(
                           ...
                           Option[ConcurrentOutputWriterSpec]): WriteTaskResult = {
    ...
    val dataWriter =
      if (sparkPartitionId != 0 && !iterator.hasNext) {
        ...
        new OmniSingleDirectoryDataWriter(description, taskAttemptContext, committer)
      } else {
        concurrentOutputWriterSpec match {
          ...
          case _ =>
            new OmniDynamicPartitionDataSingleWriter(description, taskAttemptContext, committer)
        }
      }
