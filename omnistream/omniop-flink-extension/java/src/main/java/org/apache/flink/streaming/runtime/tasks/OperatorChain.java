...
public abstract class OperatorChain<OUT, OP extends StreamOperator<OUT>>
    ...
    private OmniOperatorChain omniOperatorChain;
    ...
    public OperatorChain(
        ...
        createOmniOperatorChain(chainedConfigs);
    }
    ...
    private void createOmniOperatorChain(Map<Integer, StreamConfig> chainedConfigs) {
        System.out.println("DDBUG all StreamConfigs");
        for (Map.Entry<Integer, StreamConfig> entry : chainedConfigs.entrySet()) {
            ...
            // Print or process the StreamConfig
            System.out.println("Operator ID: " + operatorId);

            // Optionally: Further processing of StreamConfig
            // For example, getting the operator name
            String operatorName = streamConfig.getOperatorName();
            System.out.println("Operator Name: " + operatorName);
            System.out.println("Description: " + streamConfig.getDescription());
            System.out.println("");
        }
        ;
        StreamConfig mainConfig = ((AbstractStreamOperator<?>) mainOperatorWrapper.getStreamOperator()).getOperatorConfig();
        omniOperatorChain = (mainConfig.isUseOmniEnabled()) ? new OmniOperatorChain(firstOperatorWrapper) : null;
    }
