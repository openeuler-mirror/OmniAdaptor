...
public class PipelineOptions {
    ...
    public static final ConfigOption<Integer> MUISE_PARALLELISM =
            key("pipeline.muise-parallelism")
                    .intType()
                    .defaultValue(-1)
                    .withDescription(
    ...
    public static final ConfigOption<String> MUISE_API_JAR =
            key("pipeline.muise_api_jar")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(
                            "The muise api jar to package with the job jars to be sent to the"
                                    + " cluster. It has to be valid path.");
    ...
    public static final ConfigOption<String> CLOSURE_CLEANER_SKIP_CLASS_REGEX =
            key("pipeline.closure-cleaner-skip-class-regex")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(
                            "A regex of fully qualified class names; matching classes will skip ClosureCleaner checks.");
    ...
    public static final ConfigOption<Boolean> OPERATOR_REBALANCE_TO_RESCALE =
            key("pipeline.rebalance-to-rescale")
                    .booleanType()
                    .defaultValue(false)
                    .withDescription(
                            "When the parallelism of upstream and downstream operators is divisible, rebalance is converted to rescale.");
    ...
    /**
     * set operator parallel by operator id,this config may change the jobGraph topology
     */
    public static final ConfigOption<Map<String, String>> OPERATOR_PARALLELISM_OVERRIDES =
            key("pipeline.operator-parallelism-overrides")
                    .mapType()
                    .defaultValue(Collections.emptyMap())
                    .withDescription(
                            "A parallelism override map (operator id -> parallelism) which will be used to update"
                                    + " the parallelism of the corresponding job operator of submitted streamGraph.");
