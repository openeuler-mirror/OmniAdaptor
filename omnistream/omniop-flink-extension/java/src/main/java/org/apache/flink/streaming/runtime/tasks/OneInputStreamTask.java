...
public class OneInputStreamTask<IN, OUT> extends StreamTask<OUT, OneInputStreamOperator<IN, OUT>> {
    ...
    public static boolean LOW_PARA_MODE = false;

    private static int batchSize = 10000;

    private static int batchSizeOptimization = 9500;

    private static final int BATCH_SIZE_SMALL_PARRA = 10000;

    private static final int BATCH_SIZE_OPTIMIZATION_SMALL_PARRA = 12000;

    private static final int TWO_TM_PARRA = 16;
    ...
    public void init() throws Exception {
        ...
        LOW_PARA_MODE = configuration.getOmniBatchMode();
        LOG.info("omni batch mode: " + LOW_PARA_MODE);
        int parallelism = getEnvironment().getExecutionConfig().getParallelism();
        if (parallelism == TWO_TM_PARRA) {
    ...
    private static class StreamTaskNetworkOutput<IN> implements DataOutput<IN> {
        ...
        private StreamTaskNetworkOutput(
            ...
            if (this.operator.getClass().getSimpleName().equals("SinkWriterOperator")) {
                isSink = true;
            }
        }
