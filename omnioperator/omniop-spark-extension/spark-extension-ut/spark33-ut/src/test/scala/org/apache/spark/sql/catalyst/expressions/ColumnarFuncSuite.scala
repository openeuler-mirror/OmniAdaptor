...
class ColumnarFuncSuite extends ColumnarSparkPlanTest {
  ...
  protected override def beforeAll(): Unit = {
    ...
    Seq[(String, String)](
      ...
      ("123", "2"),
      (null, null)
    ).toDF("str_column1", "str_column2").createOrReplaceTempView("contains_view")
  ...
  test("Test Contains Function") {
    var res = spark.sql("select contains('123', str_column1), contains(str_column1, str_column2), " +
      "contains(str_column2, '2') from contains_view")
    ...
    res = spark.sql("select contains(x'537061726b2053514c', x'537061726b')")
