...
object InjectRuntimeFilter extends Rule[LogicalPlan] with PredicateHelper with JoinSelectionHelper {
  ...
  private def injectFilter(
    ...
    val bloomfilterSubqueryReuse = ColumnarPluginConfig.getSessionConf.enableBloomfilterSubqueryReuse
    if (conf.runtimeFilterBloomFilterEnabled) {
      val canReuseExchange = conf.exchangeReuseEnabled && bloomfilterSubqueryReuse &&
